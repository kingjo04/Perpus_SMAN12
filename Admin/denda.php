...
// Fetch data from the "pengembalian" table where the return date is overdue
$queryPengembalian = "SELECT * FROM pengembalian WHERE tenggat < CURDATE()";
...
// Iterate through the overdue returns
while ($rowPengembalian = mysqli_fetch_assoc($resultPengembalian)) {
    ...
    // Calculate the total denda (1000 Rupiah per day)
    $jumlahDenda = $hariTerlambat * 1000;

    // Insert the data into the "denda" table, including "no_hp" from "pengembalian"
    $queryInsertDenda = "INSERT INTO denda (nama, judul, start, tenggat, total, status, no_hp) VALUES ('{$rowPengembalian['nama']}', '{$rowPengembalian['judul']}', '{$rowPengembalian['tanggal']}', '{$rowPengembalian['tenggat']}', '$jumlahDenda', 'belum lunas', '{$rowPengembalian['no_hp']}')";
    mysqli_query($conn, $queryInsertDenda);
}

// Now you can proceed to fetch data from the "denda" table as you did before
